/*  backend/lib/permissions.js  — NEW FILE
    ----------------------------------------------------------------------------
    WHO CAN DO WHAT. One table, used by the API and mirrored on the frontend.

    THE RULE IN PLAIN WORDS
      · Every EcoSoch employee who is given access can read, create and update.
      · Only Admin and Super Admin can DELETE anything.
      · Only Admin and Super Admin can manage Team members and the App launcher.
      · Manager and above can reach every page. Staff see the working pages only.

    Roles are stored as free text in the Users tab (User_Role), because that tab
    already holds values like "Sales Coordinator" that were typed by hand. So
    instead of a fixed enum, every role is mapped to a TIER, and permissions are
    granted by tier. An unrecognised role falls back to STAFF, which can work but
    cannot delete — the safe direction to fail in.

    TO CHANGE SOMEONE'S ACCESS
      Open the Users tab of the Google Sheet and edit their User_Role cell to one
      of the names below. It takes effect the next time they sign in.
--------------------------------------------------------------------------- */
...
const CAPABILITY_TIERS = {
  ...
  manage_dropdowns: TIER.ADMIN,     // ← Admin screen: Project_Type, Sales_Lead, …
...
const PAGE_TIERS = {
  ...
  users      : TIER.ADMIN,
  launcher   : TIER.ADMIN,
  dropdowns  : TIER.ADMIN,   // Manage the picklists used on the project form
};
